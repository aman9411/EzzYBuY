...
function RenderDish({kid}) {
      return(
        <div className="col-12 col-md-5 m-1">
          <Card>
             <CardImg src={kid.image} alt={kid.name} />
               <CardBody>
                 <CardTitle>{kid.name}</CardTitle>
                <CardText>{kid.description} </CardText>
              </CardBody>
              </Card>
           </div>
          );
...
function RenderDetails({comments, addComment, dishId}) {
  ...
  else if(props.kid != null) {
      return (
        <div className="container">
           <div className="row">
            <Breadcrumb>
               <BreadcrumbItem><Link to='/kid'>Kids</Link></BreadcrumbItem>
               <BreadcrumbItem active>{props.kid.name}</BreadcrumbItem>
            </Breadcrumb>
